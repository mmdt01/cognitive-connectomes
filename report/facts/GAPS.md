...
|---|---|---|
| ~~**Per-variant `\|lambda_1\|` (`lambda_max_raw`) at N = 1000.**~~ **RESOLVED 25 August 2026.** The four values (**0.2333** / 0.1398 / 0.1358 / 0.1348) are now **T0 §3.1 amendment (a)**, in a table on §3.1's N = 448 columns and column order, computed read-only from `scale_1000/spectra_per_seed.parquet` and gated against §2.1's `bulk95` medians and gap ratios | chapter 4 outline item 6, "Scale", and F2 panel (b), which draws `\|lambda_1\|` at both scales | T0 §2.1, §3.1, §2.4, §2.5; A1 §2.1 to §2.5; FL F2 and S1 rows and flags; A1's F2 and S1 caption blocks |
| ~~**Per-variant absolute bulk values at N = 1000** (the four numbers behind the 6.4% spread)~~ **RESOLVED 25 August 2026.** 0.0585 / 0.0584 / **0.0590** / 0.0553 under `median(bulk95) x median(\|lambda_1\|)`, the aggregation named in **T0 §3.1 amendment (a)** itself; the per-seed-product column (0.0585 / 0.0594 / 0.0577 / 0.0554, spread 6.9%) is carried beside it and must not be mixed in. Note the ordering is **not** the N = 448 ordering: degree's is largest here | as above, and F2 panel (a) | as above |
