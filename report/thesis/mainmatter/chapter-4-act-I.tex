...
graph. Non-negativity gives $\mathbf{W}$ a real, positive leading eigenvalue, largest in
...
\begin{figure}[htbp]
    ...
    \textbf{The connectome's weight assignment buys a large spectral gap.} Eigenvalues of the un-rescaled recurrent matrix $\mathbf{W}$ on the
...
Figure~\ref{fig:F1} draws the eigenvalues of the un-rescaled matrix $\mathbf{W}$, one
...
\begin{table}[ht]
    ...
    $|\lambda_1|$ and the absolute bulk radius are in raw units of $\mathbf{W}$; the
