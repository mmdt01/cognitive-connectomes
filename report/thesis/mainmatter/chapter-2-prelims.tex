...
\begin{figure}[ht]
    \centering
    \includegraphics[width=1\textwidth]{figures/Eigenspectra.png}
    \caption[What the spectrum tells us about dynamics]{%
    \textbf{What the spectrum tells us about dynamics.} Each spectrum is drawn in
    the complex plane with the unit circle marked in grey.
    \textbf{(a)} \textit{Stability and regime.} Three operating points and the
    activity each produces. Every mode evolves as $\lambda_i^{t}$, so activity
    decays when the spectral radius $\rho(\mathbf{W})$ lies below one, is sustained
    at one, and grows above it. The spectral radii are exaggerated relative to the
    traces beneath, which would otherwise differ by several orders of magnitude over
    the window shown.
    \textbf{(b)} \textit{Timescales.} Two spectra of equal radius but different
    arrangement. Clustered eigenvalues share a modulus and so decay under a common
    envelope (dashed), differing only in frequency and giving synchronised activity;
    spread eigenvalues decay at staggered rates, so fast modes vanish while slow
    ones persist.
    \textbf{(c)} \textit{Dominant directions.} Two spectra sharing a leading
    eigenvalue $\lambda_1$ but differing in its separation from the bulk, with the
    activity of five units shown against the leading mode alone (dashed). A wide gap
    leaves the units decaying in parallel, all following one shared pattern and
    differing only in how strongly they load onto it; a narrow gap leaves them
    oscillating independently well beyond it. Both spectra are non-negative, so
    $\lambda_1$ is real and its mode is a common mode across the population.}
