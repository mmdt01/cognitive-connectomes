...
# Act III figure set
# ---------------------------------------------------------------------------
def placement_levels(scale: int = common.SCALE, spectral_radius: float = 6.0,
                     sign_mode: str = "dale") -> pd.DataFrame:
    """Absolute MC by (variant, targeting, f) — the levels behind the placement `f*`.

    Reads the frozen phase capture, which is the only source carrying all three
    targetings. MC rather than `d_eff`: at sigma = 6 every variant is against the
    `d_eff = N = 448` ceiling for f >= 0.2, which is half of why the original reading
    was exposed (TIER0 §3.8).
    """
    df = pd.read_parquet(common.phase_cells_path(scale),
                         columns=["sign_mode", "targeting", "f", "variant",
                                  "spectral_radius", "seed", "draw", "task", "mc"])
    df = df[(df.sign_mode == sign_mode) & (df.task == "mc")
            & np.isclose(df.spectral_radius, spectral_radius)]
    seeds = (df.groupby(["variant", "targeting", "f", "seed"], as_index=False)
             .mc.mean())
    return (seeds.groupby(["variant", "targeting", "f"], as_index=False).mc.median())


def figures(scale: int = common.SCALE) -> None:
    """Build the Act III figure set from artifacts already on disk."""
    from experiments.human.analysis.criticality_matched import heatmaps, panels

    common.FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    front = pd.read_parquet(common.RESULTS_DIR / f"e03_frontier_scale_{scale}.parquet")
    print("Fig A -- dissociation in levels and deltas")
    panels.fig_dissociation(front, common.FIGURES_DIR / "figA_dissociation_levels")

    print("Fig B -- the two-axis methods figure")
    results = {axis: heatmaps.run(scale, axis, "extension")
               for axis in ("nominal", "effective")}
    panels.fig_heatmaps(results, common.FIGURES_DIR / "figB_two_axis_methods",
                        methods_framing=True)

    print("Fig C -- mechanism (regenerated by --mechanism)")

    print("Fig D -- placement in levels")
    panels.fig_placement_absolute(placement_levels(scale),
                                  common.FIGURES_DIR / "figD_placement_levels")
